			...
			while($fet=mysql_fetch_array($set))
			{
				...
				if($final_amount>0){
					if($due<=$final_amount){
						...
						mysql_query("update `sales_invoice` SET `cash_amount`='$cash_amount',`due_amount`='$due_amount' where `id`='$bill_id'");
...
<title>Customer Basis Report</title>
</head>
		...
		<div class="page-content">
			<section class="content">
				<div class="row">
					<div class="col-md-12">
						...
						<div class="portlet-title ">
								<div class="caption">
									<i class="fa fa-gift"></i>Search Customer
								</div>
							</div>
							<div class="portlet-body form">
							<form method="post" action="customer_report.php?mode=view" class="form-horizontal">
									<table class="table">
										<tr>
										<td>Select Customer</td><td>:</td>
										<td>
										<select name="customer_id" class="form-control input-large select_customer" >
										<option value="">----------Choose Customer ----------</option>
											<?php
												$item_data=mysql_query("select `customer_id` from `sales_invoice` group by `customer_id`");
												while($row=mysql_fetch_array($item_data))	
												{
													$c_id=$row['customer_id'];
													?>		
											<option value="<?php echo $row['customer_id']?>">
											<?php echo fetchcustomername($c_id); ?></option>
											<?php }?>
										</select>
										</td>									
										</tr>
			 ...
			 <?php if($_GET['mode']=='view') { ?>
							...
							<div class="portlet-title ">
								<div class="caption">
									<i class="fa fa-gift"></i>Customer Sale Records
								</div>
							</div>
							<div class="portlet-body form">														
		<?php
						...
						if($q1=="" )
						{
						$qry="SELECT * from `sales_invoice` ";
						}
						else
						{
							$qry="SELECT * from `sales_invoice` WHERE ";
						}
						$q2=" group by `id`";
						 $sql=$qry.$q1.$q2;
				...
				<?php  }?>
											...
											<form method="post"  class="form-horizontal">
												...
												<?php if((!empty($customer_id)) && ($total_due>0)) { ?>
